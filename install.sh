...
rm -rf galib247
tar -xf galib247.tgz
find galib247 -type f -exec sed -i "s/float/double/g" {} \;
sed -i 's|$(INSTALL) $(LIB) $(LIB_DEST_DIR)|$(MKDIR) $(LIB_DEST_DIR)\n\t$(MKDIR) $(HDR_DEST_DIR)\n\t$(INSTALL) $(LIB) $(LIB_DEST_DIR)|' galib247/ga/makefile
sed -i 's|TMPDIR=/var/tmp|TMPDIR=.|' galib247/makefile
sed -i 's|DESTDIR=/usr/local|DESTDIR=../|' galib247/makevars 
sed -i 's|CXXFLAGS    = -g -Wall|CXXFLAGS    = -g -fpermissive -Wall|' galib247/makevars 
if [ $CXX == "g++" ]; then
  echo " " 
elif [ $CXX == "icpc" ]; then
